-- ============================================================
-- TaxDesk OS — 20260726130000 Senior/super-senior OLD-regime block narrowed
-- (K4-05)
--
-- `K4-05` closed spec §10.2's residual gap: the Section 80D "insured party"
-- (parent-premium) sub-case is now computed as an independent
-- `"80D_PARENTS"` bucket (migration 20260726120000; rules.ts
-- `deductionCapForSection`), completing the FULL later-computation dossier
-- (the k4-senior-treatment-specification design notes §10.1-§10.4): the
-- OLD-regime basic-exemption widening (K4-02), the Section 80D self/family
-- AND parents caps (K4-03/K4-05), the Section 80TTA-vs-80TTB mutual
-- exclusivity (K4-03), and the Section 207(2) advance-tax exemption
-- disclosure (K4-04) are all now real. No further senior-specific
-- computation gap is known for the population this blocker gates (a
-- resident senior/super-senior TAXPAYER) — see decision D79 in
-- the k3-tax-intelligence-program design notes §6 for the full reasoning,
-- including why a separate, newly-noted gap (a below-60 taxpayer's
-- self/family "80D" bucket not widening for a senior SPOUSE) does NOT keep
-- this blocker's condition from narrowing (it affects a population this
-- blocker never gated in the first place).
--
-- WHAT CHANGES: `app.tax_case_senior_treatment_blocked_for_case_regime`'s
-- OLD-regime branch, which UNCONDITIONALLY returned
-- 'SENIOR_TREATMENT_UNSUPPORTED' for a resident senior/super-senior
-- taxpayer regardless of ledger content, is REMOVED — selecting the OLD
-- regime is no longer blocked SOLELY for age, mirroring how the NEW regime
-- already behaved. `RESIDENTIAL_STATUS_UNRESOLVED` (unresolved residency
-- fails closed) is COMPLETELY UNCHANGED — the only remaining path this
-- function can return non-null. `app.tax_case_senior_treatment_blocked_for_manifest`
-- is untouched (thin wrapper; its behavior changes only because the function
-- it calls does). The three guarded RPCs (create_evidence_manifest,
-- capture_client_approval, finalize_tax_case) keep their senior-treatment
-- check wired exactly as before (defense in depth is not removed) — only the
-- now-corrected exception message is updated to describe what the check can
-- STILL catch (residency unresolved), since the old "Section 80D parent
-- premium" wording would now be FALSE for the only case that can still
-- reach it. No other check in any of the three functions changes. No
-- table/grant/ownership change beyond what CREATE OR REPLACE FUNCTION
-- preserves.
--
-- `REGIME_COMPARISON_UNRELIABLE` (the pre-approval, non-blocking TypeScript-
-- side disclosure) is a SEPARATE, TypeScript-only concern with no SQL
-- mirror — see `src/lib/tax-desk/senior-treatment.ts`, updated in the SAME
-- session change, not this migration.
--
-- Local Supabase only; NOT deployed, NOT run against production.
-- ============================================================

-- ------------------------------------------------------------
-- 1. app.tax_case_senior_treatment_blocked_for_case_regime — the OLD-regime
--    branch is REMOVED. Every other branch (residential status unresolved /
--    non-resident / age-undelivered / below-60) is UNCHANGED.
-- ------------------------------------------------------------
...
begin
  ...
  -- K4-05: the OLD-regime block is REMOVED. §10.1-§10.4 of the
  -- later-computation dossier are all now real (basic-exemption widening,
  -- Section 80D self/family + parents caps, Section 80TTA-vs-80TTB,
  -- Section 207(2) disclosure) — no known senior-specific computation gap
  -- remains for a resident senior/super-senior taxpayer at EITHER regime.
  return null;
end;
$$;
...
-- ------------------------------------------------------------
-- 2. create_evidence_manifest — message-text-only change to the
...
--    unchanged from 20260725160000). The check itself stays wired — it can
--    now only fire for RESIDENTIAL_STATUS_UNRESOLVED.
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- 3. capture_client_approval — message-text-only change to the
--    senior-treatment re-check (full function body replaced; every existing
--    check, including the message wording for every OTHER exception, is
--    unchanged from 20260725160000).
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- 4. finalize_tax_case — message-text-only change to the senior-treatment
...
-- ------------------------------------------------------------
-- 5. Ownership / grants (defense in depth — reasserted explicitly rather
