-- ============================================================
-- TaxDesk OS — 20260725160000 Senior/super-senior blocker message correction
-- (K4-03)
--
-- K4-03 implemented the SECOND and THIRD later-computation dossier items
-- (the k4-senior-treatment-specification design notes §10.2/§10.3): the
-- Section 80D senior-citizen cap (taxpayer's OWN age band only) and the
-- Section 80TTA-vs-80TTB mutual-exclusivity cap correction are now real
-- (src/lib/tax-engine/ay-2026-27/rules.ts `deductionCapForSection`, applied
-- by `compute-tax.ts`'s `computeDeductions` for a RESIDENT taxpayer's
-- OLD-regime figure only). It also closed the K4-02 dossier's second,
-- distinct finding: a below-60 taxpayer's "80TTB" ledger row is no longer
-- accepted at the senior ₹50,000 cap.
--
-- Per the K4-03 decision (D70), the OLD-regime `SENIOR_TREATMENT_UNSUPPORTED`
-- block (app.tax_case_senior_treatment_blocked_for_case_regime — UNCHANGED by
-- this migration, still returns the SAME code for the SAME condition)
-- DELIBERATELY STAYS in place: Section 80D's higher cap for a premium paid on
-- behalf of a senior/super-senior PARENT (while the taxpayer themselves may be
-- below 60) is NOT modelled — the ledger carries one undifferentiated "80D"
-- bucket with no field distinguishing whose premium is paid — so an
-- OLD-regime senior/super-senior computation is still not reliably complete.
-- What changes here is ONLY the exception MESSAGE text in the three guarded
-- RPCs that raise it (create_evidence_manifest, capture_client_approval,
-- finalize_tax_case) — the pre-K4-03 wording ("the Section 80D senior-citizen
-- cap and Section 80TTB/80TTA treatment are not [applied]") is now FALSE
-- (both ARE now applied, for the taxpayer's own age band) and is replaced
-- with wording that names the actual remaining gap. No other check in any of
-- the three functions changes. No table/column/grant/ownership change —
-- ADDITIVE message-text-only correction. Local Supabase only; NOT deployed,
...
begin
  ...
  -- K4-01/K4-02/K4-03: the senior/super-senior selected-regime gate. Re-derived
  ...
  -- again in K4-03 once the 80D/80TTA/80TTB self-case gaps became real.
  ...
  if v_senior_blocked is not null then
    raise exception 'Not eligible for an evidence manifest under the % regime (%). Senior/super-senior old-regime basic-exemption treatment and age-gated Section 80D / 80TTA-80TTB deductions are now applied based on the taxpayer age band, but Section 80D does not yet apply a higher cap when the premium is paid on behalf of a senior parent — reliance on the OLD regime remains blocked for this case.',
...
begin
  ...
  -- K4-01/K4-02/K4-03: defense-in-depth re-check — the manifest could only
  -- exist already-clear of this block (create_evidence_manifest refuses it),
  -- but this RPC never trusts that without re-deriving it directly. CONDITION
  -- unchanged from K4-01; only the message below was corrected again in
  -- K4-03.
  v_senior_blocked := app.tax_case_senior_treatment_blocked_for_manifest(p_manifest_id);
  if v_senior_blocked is not null then
    raise exception 'This tax case is not approval-ready (%). Senior/super-senior old-regime basic-exemption treatment and age-gated Section 80D / 80TTA-80TTB deductions are now applied based on the taxpayer age band, but Section 80D does not yet apply a higher cap when the premium is paid on behalf of a senior parent — reliance on the OLD regime remains blocked for this case.', v_senior_blocked
      using errcode = '22023';
  end if;
...
begin
  ...
  -- K4-01/K4-02/K4-03: re-check against the case's CURRENTLY bound review
  -- manifest, when one exists. A case without any bound manifest has no
  -- regime-specific senior-treatment fact to re-check here (filing readiness
  -- / client review already require a current approval bound to a manifest
  -- before finalize is offered in the UI) — this is defense-in-depth on top
  -- of that, not a new independent gate for the no-manifest case. CONDITION
  -- unchanged from K4-01; only the message below was corrected again in
  -- K4-03.
  if v_review_manifest_id is not null then
    ...
    if v_senior_blocked is not null then
      raise exception 'Cannot finalize (%). Senior/super-senior old-regime basic-exemption treatment and age-gated Section 80D / 80TTA-80TTB deductions are now applied based on the taxpayer age band, but Section 80D does not yet apply a higher cap when the premium is paid on behalf of a senior parent — reliance on the OLD regime remains blocked for this case.', v_senior_blocked
        using errcode = '22023';
    end if;
  end if;
